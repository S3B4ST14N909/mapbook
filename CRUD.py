...
def remove_user_db(db_params) -> None:
    cursor = db_params.cursor()
    sql = f"DELETE FROM public.users WHERE name='{input('Enter name of the person to delete:')}'"
    cursor.execute(sql)
    db_params.commit()
    cursor.close()
...
def update_db(db_params) -> None:
    ...
    new_posts: int = int(input('Enter new number of your posts:'))
    new_location: str = input('Enter your new location:')
    longitude, latitude = get_coordinates(new_location)
    cursor = db_params.cursor()
    sql = f"UPDATE public.users SET name='{new_name}', surname='{new_surname}', posts = {new_posts}, location='{new_location}', coords='SRID=4326;POINT({latitude} {longitude})' WHERE name='{input('Enter name of the person to update:')}';"
