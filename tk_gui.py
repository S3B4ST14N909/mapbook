...
def user_details():
    ...
    label_name_sczegoly_obiektu_wartosc.config(text=users[i].name)
...
button_usun_obiekt = Button(ramka_lista_obiektow, text='Usuń obiekt', command=remove_users)
...
label_name_sczegoly_obiektu = Label(ramka_szczegoly_obiektow, text='Imię: ')
label_name_sczegoly_obiektu_wartosc = Label(ramka_szczegoly_obiektow, text='... ', width=10)
...
label_name_sczegoly_obiektu.grid(row=1, column=0)
label_name_sczegoly_obiektu_wartosc.grid(row=1, column=1)
...
map_widget.grid(row=1, column=0, columnspan=8)
